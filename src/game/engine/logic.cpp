...
void LogicClass::AI(void)
{
    ...
    for ( LogicTriggerID = 0; LogicTriggerID < LogicTriggers.Count(); ++LogicTriggerID ) {
        ...
        //DEBUG_LOG("LogicClass::AI() - LogicTriggerID is %d, trigger name is '%s'\n", LogicTriggerID, LogicTriggers[LogicTriggerID]->ObjectType->Get_Name());
        ...
        if ( trigger != nullptr ) {
            ...
            if ( (!Scen._GlobalsChanged
               || !trigger->Spring(TEVENT_GLOBAL_SET))
               && !trigger->Spring(TEVENT_GLOBAL_CLEAR)

              && (!Scen._DestroyBridges
               || !trigger->Spring(TEVENT_ATTACHED_BRIDGE_DESTROYED))
