...
public:
    ...
    int const Encode_Modulus(void *buffer) const;
    int const Encode_Exponent(void *buffer) const;
    ...
    int const Encrypt(void *source, int slen, void *dest) const;
    int const Decrypt(void *source, int slen, void *dest) const;
    int Get_Count_Bits() const;
    int const Get_Key_Length() const { return m_bitPrecision; }
    int Key_Bytes() const { return (m_bitPrecision - 1) / 8; }
    //int Block_Count(int unk);
