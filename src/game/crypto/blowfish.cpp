...
// clang-format off
const uint32_t g_PInit[BF_ROUNDS + 2] = {
...
BlowfishEngine::BlowfishEngine() : m_HasKey(false)
{
}

BlowfishEngine::BlowfishEngine(void *key, int length) : m_HasKey(false)
{
    if (key && length) {
        Submit_Key(key, length);
    }
}
...
/**
* @brief Resets the state of the blowfish engine and sets the encrption key.
*/
void BlowfishEngine::Submit_Key(void *key, int length)
{
    ...
    memcpy(m_PBoxEnc, g_PInit, sizeof(m_PBoxEnc));
    memcpy(m_PBoxDec, g_PInit, sizeof(m_PBoxDec));
    memcpy(m_SBox, g_SInit, sizeof(m_SBox));
    ...
    if (key == nullptr || length == 0) {
        ...
        int keypos = 0;
        uint8_t *keystring = static_cast<uint8_t *>(key);

        for (int i = 0; i < BF_ROUNDS + 2; ++i) {
            uint32_t data = 0;

            data = keystring[keypos++ % length];
            data = keystring[keypos++ % length] | (data << 8);
            data = keystring[keypos++ % length] | (data << 8);
            data = keystring[keypos++ % length] | (data << 8);
        ...
        uint32_t *enc = m_PBoxEnc;
        uint32_t *dec = &m_PBoxDec[BF_ROUNDS + 1];
...
/**
* @brief Encrypts data passed in.
*/
int BlowfishEngine::Encrypt(void *plaintext, int length, void *cyphertext)
{
    return Process(true, plaintext, length, cyphertext);
}

/**
* @brief Decrypts data passed in.
*/
int BlowfishEngine::Decrypt(void *cyphertext, int length, void *plaintext)
{
    return Process(false, cyphertext, length, plaintext);
}

/**
* @brief Encrypts or Decrypts data passed in depending on bool.
*/
int BlowfishEngine::Process(bool encrypt, void *in_text, int length, void *out_text)
{
    uint8_t *in = static_cast<uint8_t *>(in_text);
    uint8_t *out = static_cast<uint8_t *>(out_text);

    if (in_text == nullptr || length == 0) {
        return 0;
    }
    ...
    uint32_t *box = encrypt ? m_PBoxEnc : m_PBoxDec;

    if (m_HasKey) {
        int blocks = length / BF_BLOCKSIZE;

        for (int i = 0; i < blocks; ++i) {
            Process_Block(in, out, box);
            in += BF_BLOCKSIZE;
            out += BF_BLOCKSIZE;
        }

        int blocks_size = BF_BLOCKSIZE * blocks;
        if (blocks_size < length) {
            memmove(out, in, length % BF_BLOCKSIZE);
        }
        return blocks_size;
    }
...
#define F(x) (m_SBox[3][GETBYTE(x, 0)] + (m_SBox[2][GETBYTE(x, 1)] ^ (m_SBox[1][GETBYTE(x, 2)] + m_SBox[0][GETBYTE(x, 3)])))

/**
* @brief Internal function for processing an 8 byte block.
*/
void BlowfishEngine::Process_Block(void *input, void *output, uint32_t *pbox)
{
    uint32_t *in = static_cast<uint32_t *>(input);
    uint32_t *out = static_cast<uint32_t *>(output);

    uint32_t ldata = be32toh(in[0]);
    uint32_t rdata = be32toh(in[1]);

    for (int i = 0; i < BF_ROUNDS; i += 2) {
        ldata ^= pbox[i];
        rdata ^= pbox[i + 1] ^ F(ldata);
        ldata ^= F(rdata);
    }

    rdata ^= pbox[BF_ROUNDS + 1];
    ldata ^= pbox[BF_ROUNDS];
...
/**
* @brief Internal function for processing the key material.
*/
void BlowfishEngine::Sub_Key_Encrypt(uint32_t &left, uint32_t &right)
{
    uint32_t ldata = left;
    uint32_t rdata = right;

    for (int i = 0; i < BF_ROUNDS; i += 2) {
        ldata ^= m_PBoxEnc[i];
        rdata ^= m_PBoxEnc[i + 1] ^ F(ldata);
        ldata ^= F(rdata);
    }

    left = rdata ^ m_PBoxEnc[BF_ROUNDS + 1];
    right = ldata ^ m_PBoxEnc[BF_ROUNDS];
}
