...
BufferClass::BufferClass(int size) : m_buffer(nullptr), m_size(size), m_allocated(false)
{
    if (size > 0) {
...
BufferClass::BufferClass(void *buff, int size) : m_buffer(static_cast<uint8_t *>(buff)), m_size(size), m_allocated(false)
{
...
BufferClass::BufferClass(BufferClass const &that) : m_buffer(that.m_buffer), m_size(that.m_size), m_allocated(false)
{
    // empty
}
...
{
    return m_buffer == that.m_buffer && m_size == that.m_size && m_allocated == that.m_allocated;
}
